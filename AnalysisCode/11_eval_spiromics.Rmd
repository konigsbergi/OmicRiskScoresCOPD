...
knitr::opts_knit$set(root.dir = "C:/Users/konigsbi/Desktop/ors_topmed/January/Results/predictions_spiromics/")
```
...
setwd("C:/Users/konigsbi/Desktop/ors_topmed/January/Results/predictions_spiromics/")
files = dir()
...
dim(prot) #2136
#sooooo, samps in rows. features in cols. 
...
#looks like pd now has multiples of a lot of lads....
#probably need to address across spiromics scripts. 


#make sure orders are logical
...
should be good to go on pd set up. sweet. 


add preds to pd
i cant have rna combined with others right? nope. so complicated. 
```{r}
dir = "C:/Users/konigsbi/Desktop/ors_topmed/January/Results/predictions_spiromics/"
files = dir(dir)
...
#wait, this worked in mesa. is not making much sense here. 
x = files[1]
comb = data.frame()
for (x in files){
  #x = files[1]
...
#miss = apply(comb_rna, 1, function(x){sum(is.na(x))})
#comb_rna = comb_rna[miss < 25, ] #
...
dim(comb) #2250. i think thats good.
dim(comb_rna) #1840. perfect. 


```

add in new spiromics models
will prolly need to update. 
...
#6mw? its weirdly standardizwd..... i guess used. SIX_MINUTE_WALK_DISTANCE_A 
...
df = rbind(df, c("BODE", summary(mod)$coefficients[2, "Pr(>|z|)"], terms$estimate[1], terms$conf.low[1], terms$conf.high[1], part, NA, NA)) 
#prolly dont want to use rsq.....
#get a worrisome warning about num of columns...
...
write.csv(df, "C:/Users/konigsbi/Desktop/ors_topmed/manuscript/Supplement/Full_SPIROMICS_forSupp.csv")
...
png("C:/Users/konigsbi/Desktop/ors_topmed/April/Figures/SPIROMICS_replication.png")
...
```{r}
#load MESA
load("C:/Users/konigsbi/Desktop/ors_topmed/April/Figures/mesa_rep.Rdata")
...
png("C:/Users/konigsbi/Desktop/ors_topmed/April/Figures/Figure3.png", units = "in", res = 500, height = 15, width = 12)
