...
#remove p3 scores. 
#files = files[grep("P2", files)]
#keeping for phenos

#removing gold category as wrong size files...
files = files[-grep("GoldCategory", files)]
...
#what do about smoking status?

#i should adjust for baseline for change traits. ugh. how do i work that in?


#pd = pd %>% dplyr::select(sid, all_of(num_vars), all_of(log_vars), all_of(bin_vars), all_of(negbinom_vars), all_of(covars))
rownames(pd) = pd$sid
...
write.csv(full, "C:/Users/konigsbi/Desktop/ors_topmed/January/Results/COPDGene_testing_results.csv")
```
...
#volcano plot. no negative betas, doesnt look like vlocano. 
#groups by model type, so should subset. or facet. 
full$class2 = full$class
#full[full$class2 == "log" | full$class2 == "gaussian", ]$class2 = "Continuous"
#full[full$class2 == "negbinomial" | full$class2 == "binomial", ]$class2 = "Non-Continuous"
...
write.csv(full, "C:/Users/konigsbi/Desktop/ors_topmed/manuscript/Supplement/Full_COPDGene_forSupp.csv")
...
```{r}
modfiles = dir("C:/Users/konigsbi/Desktop/ors_topmed/January/Results/models_copdgene/")
...
modfiles = paste0("C:/Users/konigsbi/Desktop/ors_topmed/January/Results/models_copdgene/", modfiles)

#make sure model_select cant select zero feature models.............................. what do zero feature models even mean. 
...
table(df$V4)
table(df$V5)
median(df$V2) #also ran the above with with get_model_best to get median for best models. 144.5 total. 675. 
median(df[df$V3 == "metabolite", ]$V2) # 177. 534
median(df[df$V3 == "protein", ]$V2) # 135.5. 653.4
median(df[df$V3 == "transcript", ]$V2) #82. 2793.5

#FEATURE NUMBER
#great. zoomed in so i dont have to worry about that outlier. 
...
#ALPHA (1SE)
#now plot alpha selection. also include for like optimal, not 1se. 
#so this is fuck ugly...... how do i make this less fuck ugly....
#should prolly add ticks by alphas tested....
...
#supp fig
png("C:/Users/konigsbi/Desktop/ors_topmed/April/Figures/SuppFigure2.png", units = "in", res = 500, height = 4.5, width = 9)
...
png("C:/Users/konigsbi/Desktop/ors_topmed/April/Figures/Figure1.png", units = "in", res = 500, height = 13, width = 13)
...
#example alpha/lambda selection. find optimal vals for vertical lines. figure out putting vertical lines on plot. 
#select fev1 protein
mod = readRDS("C:/Users/konigsbi/Desktop/ors_topmed/January/Results/models_copdgene/model_protein_FEV1_post_P2.rds")
index = 1
df = NULL
for (x in 1:11){
  alpha = mod$alpha[x]
...
png("C:/Users/konigsbi/Desktop/ors_topmed/April/Figures/SuppFigure3.png", units = "in", width = 8, height = 6, res = 500)
x = mod
log.x = T
 n <- length(x$modlist)
...
write.csv(df, "C:/Users/konigsbi/Desktop/ors_topmed/manuscript/Supplement/ST_modelparams.csv")
...
```{r eval=F}
#have full generated from above. 
#need to cut down on things. 

ggplot(full, aes(type, beta)) +
  geom_boxplot()
  geom_beeswarm() #not impressive. 

summary(aov(beta ~ type, full)) #0.62
...
summary(aov(beta ~ type, sub)) #4.64e-05 ***

t.test(beta ~ type, sub[sub$type != "Metabolite", ]) #2.214e-05. so prot and trans dif. 
t.test(beta ~ type, sub[sub$type != "Transcript", ]) #0.02694 
t.test(beta ~ type, sub[sub$type != "Protein", ]) #0.01216
...
summary(aov(-log10(p) ~ type, sub)) #0.000239 ***

t.test(-log10(p) ~ type, sub[sub$type != "Metabolite", ]) #9.564e-05
t.test(-log10(p) ~ type, sub[sub$type != "Transcript", ]) #0.2732
t.test(-log10(p) ~ type, sub[sub$type != "Protein", ]) #0.002286
